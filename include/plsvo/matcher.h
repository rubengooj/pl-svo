...
public:
  ...
  /// Find a match for a line segment by directly applying subpix refinement in the extreme points.
  /// IMPORTANT! This function assumes that px_cur is already set to an estimate that is within ~2-3 pixel of the final result!
  bool findMatchDirect(
      const LineSeg& ls,
      const Frame& frame,
  ...
  /// Find a match by searching along the epipolar line without using any features.
  bool findEpipolarMatchDirectSegmentEndpoint(
      const Frame& ref_frame,
      const Frame& cur_frame,
      const Feature& ref_ftr,
      const double d_estimate,
  ...
  /// Compute affine warping according to current parameters and the feature position
  int precomputeRefPatch(
      const Vector3d& pos,
      const Vector2d& px,
      const Vector3d& f,
      const Frame& cur_frame);

  /// Copy inner content from patch with border to patch
