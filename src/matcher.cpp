...
#include <plsvo/feature_alignment.h>
    ...
    Matrix2d& A_cur_ref)
{
  ...
  // Central 3D point in reference frame
  const Vector3d xyz_ref(f_ref*depth_ref);
  // Bearing vector for points in right and botton of the patch
  ...
  // Bearing vectors are normalized to lie in the same Z-plane as the central 3D point
  xyz_du_ref *= xyz_ref[2]/xyz_du_ref[2];
  xyz_dv_ref *= xyz_ref[2]/xyz_dv_ref[2];
  // Transform the triplet of points to the current frame and project them into image points (in pixels)
  const Vector2d px_cur(cam_cur.world2cam(T_cur_ref*(xyz_ref)));
  const Vector2d px_du(cam_cur.world2cam(T_cur_ref*(xyz_du_ref)));
  const Vector2d px_dv(cam_cur.world2cam(T_cur_ref*(xyz_dv_ref)));
  // Build affine transformation from deformation of the original square
    ...
    uint8_t* patch)
{
  ...
  // The approach is inverse: We assume that the *result* patch after affine transformation is square
  // and its elements are px_patch. We then warp back by the affinity the locations in this *final* patch
  // to its original pixel location in the not-warped image, and interpolate the intensity values
  // from this original location
  // TODO: A segment patch is not square, but RotatedRect
  //       Nevertheless, once the segment is warped (transformation of the end points)
  //       and the ROI determined, the same approach as here can be used
  //       to fill the intensity values in the ROI pixels
  ...
  for (int y=0; y<patch_size; ++y)
  {
    for (int x=0; x<patch_size; ++x, ++patch_ptr)
    {
      ...
      const Vector2f px(A_ref_cur*px_patch + px_ref_pyr); // linear transformation here on local (patch) coordinates + translation
...
int Matcher::precomputeRefPatch(
    const Vector3d& pos,
    const Vector2d& px,
    const Vector3d& f,
    const Frame& cur_frame)
{
  // warp affine
  warp::getWarpMatrixAffine(
      *ref_ftr_->frame->cam_, *cur_frame.cam_, px, f,
      (ref_ftr_->frame->pos() - pos).norm(),
      cur_frame.T_f_w_ * ref_ftr_->frame->T_f_w_.inverse(), ref_ftr_->level, A_cur_ref_);
  search_level_ = warp::getBestSearchLevel(A_cur_ref_, Config::nPyrLevels()-1);
  // is img of ref_frame fully available at any time? that means keeping stored previous images, for how long?
  warp::warpAffine(A_cur_ref_, ref_ftr_->frame->img_pyr_[ref_ftr_->level], px,
                   ref_ftr_->level, search_level_, halfpatch_size_+1, patch_with_border_);
  // patch_with_border_ stores the square patch (of pixel intensities) around the reference feature
  // once the affine transformation is applied to the original reference image
  // the border is necessary for gradient operations (intensities at the border must be precomputed by interpolation too!)
...
{
  // get reference feature in closest view (frame)
  if(!ls.getCloseViewObs(cur_frame.pos(), ref_ftr_))
    return false;

  LineFeat* ref_ls_ftr_ = static_cast<LineFeat*>(ref_ftr_);
  // abort if any of the segment extreme points do not lie inside the reference image
  if(!ref_ftr_->frame->cam_->isInFrame(
      ref_ls_ftr_->spx.cast<int>()/(1<<ref_ftr_->level), halfpatch_size_+2, ref_ftr_->level) ||
     !ref_ftr_->frame->cam_->isInFrame(
      ref_ls_ftr_->epx.cast<int>()/(1<<ref_ftr_->level), halfpatch_size_+2, ref_ftr_->level))
    return false;

  bool success = true;
  // Optimize start point
  {
    search_level_ = precomputeRefPatch(ls.spos_, ref_ls_ftr_->spx, ref_ls_ftr_->sf, cur_frame);

    // optimize feature position in the search_level_ of the pyramid
    Vector2d px_scaled(spx_cur/(1<<search_level_));
    success = success & feature_alignment::align2D(
          cur_frame.img_pyr_[search_level_], patch_with_border_, patch_,
          options_.align_max_iter, px_scaled);
    spx_cur = px_scaled * (1<<search_level_);
  }
  // Optimize end point
  {
    search_level_ = precomputeRefPatch(ls.epos_, ref_ls_ftr_->epx, ref_ls_ftr_->ef, cur_frame);

    // optimize feature position in the search_level_ of the pyramid
    Vector2d px_scaled(epx_cur/(1<<search_level_));
    success = success & feature_alignment::align2D(
          cur_frame.img_pyr_[search_level_], patch_with_border_, patch_,
          options_.align_max_iter, px_scaled);
    epx_cur = px_scaled * (1<<search_level_);
  }
  // If both end points were refined successfully success should remain true
  return success;
}
    ...
    double& depth)
{
  ...
  reject_ = false;
  const PointFeat* pt_ftr = static_cast<const PointFeat*>(&ref_ftr);
   if(pt_ftr->type == PointFeat::EDGELET && options_.epi_search_edgelet_filtering)
  {
    const Vector2d grad_cur = (A_cur_ref_ * pt_ftr->grad).normalized();
  ...
  if( epi_length_ < 2.0 && !std::isnan(fabsf(epi_length_)) && !std::isinf(fabsf(epi_length_)) )
  {
...
bool Matcher::findEpipolarMatchDirectSegmentEndpoint(
    const Frame& ref_frame,
    const Frame& cur_frame,
    const Feature& ref_ftr,
    const double d_estimate,
    ...
    double& depth)
{
  ...
  // feature pre-selection
  reject_ = false;
  const LineFeat* ls_ftr = static_cast<const LineFeat*>(&ref_ftr);
  ...
  /*double epi_ang_ = std::min(fabs(epi_dir_[0]),fabs(epi_dir_[1])) / std::max(fabs(epi_dir_[0]),fabs(epi_dir_[1]));
  double ang_dif_ = epi_ang_*180.0/3.1416 - ls_ftr->angle*180.0/3.1416;
  if(ang_dif_ >  180.0) ang_dif_-=360.0;
  if(ang_dif_ < -180.0) ang_dif_+=360.0;
  if( fabsf(ang_dif_) < 30.0 )   // skip lines aligned with epipolar line
  {
    reject_ = true;
    return false;
  }*/
  ...
  if( std::isnan(epi_length_) || std::isinf(epi_length_) )
  {
    reject_ = true;
    return false;
  }
