    ...
    Map* map)
{
  ...
  for(list<PointFeat*>::iterator it_ftr=frame1->pt_fts_.begin(); it_ftr!=frame1->pt_fts_.end(); ++it_ftr)
  {
    Point* pt = (*it_ftr)->feat3D;
    if(pt == NULL)
      continue;
  ...
  for(list<PointFeat*>::iterator it=frame1->pt_fts_.begin(); it!=frame1->pt_fts_.end(); ++it)
  {
    if((*it)->feat3D == NULL)
     continue;
    (*it)->feat3D->pos_ = (*it)->feat3D->v_g2o_->estimate();
    (*it)->feat3D->v_g2o_ = NULL;
  }
  ...
  for(list<EdgeContainerSE3>::iterator it_e = edges.begin(); it_e != edges.end(); ++it_e)
    if(it_e->edge->chi2() > reproj_thresh_squared)
    {
      ...
      if(it_aux->feat3D != NULL)
      {
        map->safeDeletePoint(it_aux->feat3D);
        it_aux->feat3D = NULL;
      }
      ++n_incorrect_edges;
    }
